...
#=> first make a graph in which two nodes are connected only when they have only one
#   letter difference
#=> then use BFS to find the shortest path from beginWord to endWord
        ...
        for j in range(i+1,len(wordList)):
            if len(wordList[i])==len(wordList[j]):
                for k in range(len(wordList[i])):
                    a = wordList[i][:k]+wordList[i][k+1:]
                    b = wordList[j][:k]+wordList[j][k+1:]
                    if a==b:
                        if wordList[i] in adjList.keys():
                            adjList[wordList[i]].append(wordList[j])
                        else:
                            adjList.update({wordList[i]:[wordList[j]]})
                        if wordList[j] in adjList.keys():
                            adjList[wordList[j]].append(wordList[i])
                        else:
                            adjList.update({wordList[j]:[wordList[i]]})
                        break
...
def bfs(beginWord,endWord):
    ...
    while len(queue)!=0:
        ...
        if a==endWord:
            print "path found"
            printPath(parent,endWord)
        else:
            for word in adjList[a]:
                if word not in visited:
                    queue.append(word)
                    visited.update({word:True})
                    parent.update({word:a})

def printPath(parent,endWord):
    print "Path is:"
    a = endWord
    print endWord,"<-",
    count = 1
...
def ladderLengthBFSwithoutGraph(beginWord, endWord, wordList):
    ...
    if beginWord not in wordList or endWord not in wordList:
        print "BeginWord or EndWord not in WordList"
        return 0
    #applying bfs
